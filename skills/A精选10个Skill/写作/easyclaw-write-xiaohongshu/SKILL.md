...
flowchart TD
  ...
  F --> G{用户是否提供图片?}
  G -- 是 --> H[用用户图片]
  G -- 否 --> I[image-gen 生成 1~2 张 9:16 图]
  H --> J[用小红书MCP发布]
  I --> J
```
...
## 步骤 5：若用户没给图，使用 image-gen 生成 1～2 张高清图（9:16 优先）

**目标**：根据文案主题生成原创配图，避免版权风险。

### 调用 image-gen skill 生成图片

使用以下命令调用 image-gen 生成图片：

```bash
python C:\Users\CF\.easyclaw\skills\image-gen\scripts\generate_image.py --prompt "<图片描述>" --filename "<输出路径>.jpg" --aspect-ratio 9:16
```

### 图片生成要求：

- **数量**：1～2 张即可
- **比例**：优先 9:16（竖图），使用 `--aspect-ratio 9:16`
- **分辨率**：使用 `--resolution 2K`（默认）或 `3K`
- **格式**：输出必须为 JPEG 格式（`.jpg` 或 `.jpeg`）
- **风格**：与文案主题强相关，干净、高清
...
| 主题 | 提示词示例 |
|------|----------|
| 带饭/便当 | "精美的便当盒装食物，日式风格，自然光线，9:16 竖图" |
| 化妆/美妆 | "化妆品摆放，化妆台，柔和光线，ins 风格，9:16 竖图" |
| 美食 | "美食特写，俯拍视角，温暖色调，9:16 竖图" |
| 穿搭 | "日常穿搭，街拍风格，自然光，9:16 竖图" |
| 学习 | "书桌学习场景，笔记本和笔，温馨氛围，9:16 竖图" |
...
如果当前环境没有 Firecrawl MCP、小红书 MCP 或 image-gen：

- 仍按流程完成"分析→写作"，配图部分使用 image-gen 命令占位
- 把需要用户补充的信息列成最短清单（例如：给 3~5 个对标帖子链接，或授权/接口配置）
- 如 image-gen 不可用，提供 Pexels/Unsplash 搜索链接作为备选方案
...
- 输出标题+正文+标签，并做字数自检
- 没有图片就使用 image-gen 生成 1~2 张高清 9:16 竖图
- 用小红书 MCP 发布（若可用）
