...
When the workflow produces a **saved file** (research report, summary, index, or any markdown artifact from this skill):

- **Directory**: `outputs/` inside the folder that contains this skill — i.e. **`deerflow-deep-research/outputs/`** (same directory level as `SKILL.md`).
- **Naming**: Use a clear, descriptive basename (e.g. `topic-slug.md`, or include date when useful: `topic-2026-04-10.md`).
- **Do not** place final deliverables only in chat; persist them under `deerflow-deep-research/outputs/` unless the user explicitly asks for another path.
