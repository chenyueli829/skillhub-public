---
name: vercel-deploy
description: 将应用程序和网站部署到 Vercel。当用户请求部署操作时使用此技能，例如"部署我的应用"、"将这个部署到生产环境"、"创建预览部署"、"部署并给我链接"或"上线"。无需身份验证——返回预览 URL 和可认领的部署链接。
metadata:
  author: vercel
  version: "1.0.0"
---
...
对于静态 HTML 项目（无 `package.json`），框架设为 `null`。

## 静态 HTML 项目

对于没有 `package.json` 的项目：
- 如果存在一个非 `index.html` 命名的单个 `.html` 文件，会自动重命名
- 这确保页面在根 URL（`/`）上提供服务
...
如果部署因网络限制失败（在 claude.ai 上常见），请告知用户：
